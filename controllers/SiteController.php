...
public function actionCreate(){
    ...
    if(!$toDo){
        Yii::$app->session->setFlash('error', 'Please fill todo name.');
        return $this->redirect(['index']);
        exit;
    }
    $category_id = Yii::$app->request->post('category_id');
    if(!$category_id){
        Yii::$app->session->setFlash('error', 'Please choose category.');
        return $this->redirect(['index']);
        exit;
    }

    $saved = (new \yii\db\Query())
        ->createCommand()
        ->insert('todo', [
            'name' => $toDo,
        ...
        if($saved){
        Yii::$app->session->setFlash('success', 'Inserted.');
            return $this->redirect(['index']);
        }
}
